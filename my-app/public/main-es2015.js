...
let BreakfastsComponent = class BreakfastsComponent {
    constructor(waiqueenservice) {
...
BreakfastsComponent.ctorParameters = () => [
...
let BurguerComponent = class BurguerComponent {
...
BurguerComponent.ctorParameters = () => [
    { type: _services_waiqueen_service__WEBPACK_IMPORTED_MODULE_2__["WaiqueenService"] }
];
...
let MesasComponent = class MesasComponent {
...
let PedidosComponent = class PedidosComponent {
    ...
    ngOnInit() {
        this.waiqueenservice.getOrder().subscribe((resp) => {
            this.order = resp;
...
let WaiqueenService = class WaiqueenService {
    constructor(afs) {
        //------------------------------ Mesas -------------------------------------------------//
        this.afs = afs;
        ...
        this.arrayTabls = this.tablsCollection.snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(actions => actions.map(a => {
        ...
        //Guardo coleccion en array
        this.arraymens = this.menuCollection.snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(actions => actions.map(a => {
            const data = a.payload.doc.data();
            const id = a.payload.doc.id;
            return Object.assign({ id }, data);
        })));
        ...
        //Guardo coleccion en array
        this.arrayOrder = this.orderCollection.snapshotChanges().pipe(Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_2__["map"])(actions => actions.map(a => {
            const data = a.payload.doc.data();
            const id = a.payload.doc.id;
            return Object.assign({ id }, data);
        })));
...
WaiqueenService.ctorParameters = () => [
